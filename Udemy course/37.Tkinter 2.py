...
from tkinter import ttk
                              ...
                              # передаёт функции переменное количество аргументов
def set_entry(*args):         # *args - для неименованых, **kwargs - для именованых аргументов
    entry_1_txt.set("Hello")       # *args - non-keyworded, **kwargs - keyworded arguments


def chk_but_changed(*args):
    entry_1_txt.set(chk_but_1_txt.get())     # associated with check button change
...
root.columnconfigure(0, weight=1)  # everything move in the same way
...
label_1.grid(column=1, row=1, sticky=(W, E))   # put label inside the grid
...
# Define the number of characters long and the StringVar it is associated with
entry_1 = ttk.Entry(frame, width=7, textvariable=entry_1_txt)
entry_1.grid(column=2, row=1, sticky=(W, E))   # put it inside grid
...
# Text to display next to it, function to call, StringVar, value assigned when check and not checked
...
# ----- RADIO BUTTONS -----
# Only 1 radio button can be selected at a time Shared StringVar
radio_but_1_val = StringVar()
# Parent, text assigned, StringVar, value assigned to StringVar, function to call on event
