...
rand_str = "Match everything up to @"
                # ^ - match beginning of the string; . - any character or space;
                # * - more; [^@] - all is not @ symbol
regex = re.compile(r"^.*[^@]")
...
# Grab the 1st word of each line using the the multiline code which allows for the
# targeting of each line after a line break with ^
rand_str = '''Ape is big
Turtle is slow
Cheetah is fast'''
...
for i in matches:
    print(i)
print()
# Subexpressions are parts of a larger expression. If you want to match for a large block,
# but only want to return part of it. To do that surround what you want with ( )
